...
let parseSeeds2 (line: string): list<Seed> =
    line.Substring 7 |>
        ...
        Seq.map int64 |>
        Seq.chunkBySize 2 |>
        Seq.map (fun xs -> {start = xs[0]; ``end`` = xs[0] + xs[1]}) |>
        Seq.toList
    
let rec groupbyHelper f curr acc xs =
    match xs with
    | [] -> if List.isEmpty acc then Seq.empty else Seq.singleton (List.rev acc)
    | y :: ys ->
        if f y = curr
            then 
...
let parseFile lines =
    match lines with
    | seeds :: rest ->
        (parseSeeds seeds, parseMaps rest)
    | _ -> failwith "parse error"
    
let parseFile2 lines =
    match lines with
    | seeds :: rest ->
        (parseSeeds2 seeds, parseMaps rest)
    | _ -> failwith "parse error"
