...
type GameSet = { id: int; games: list<Game> }
...
let unionMaps (m1: Map<'k, int>) (m2: Map<'k, int>) =
    seq {
        ...
        for (k, v) in Seq.zip m2.Keys m2.Values |> Seq.filter (fun (k, _) -> not (m1.ContainsKey k)) do
            yield (k, v)
    }
    |> Map
...
let parseEntry (s: string) =
    split s ' ' |> fun arr -> (arr.[1], int arr.[0])

let parseGame (s: string) : Game =
    split s ',' |> Seq.map parseEntry |> Map |> (fun x -> { balls = x })
...
let parseGameSet (s: string) : GameSet =
    ...
    | [| identity: string; gameSetString: string |] ->
        { id = parseID identity
          games = split gameSetString ';' |> Array.map parseGame |> Array.toList }
...
let processValidGameSets =
    Seq.filter (validGameSet actual) >> Seq.map _.id >> Seq.sum

let processSetPowers =
    Seq.map (_.games >> unionGame >> _.Values >> Seq.fold (*) 1) >> Seq.sum
