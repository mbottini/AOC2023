    ...
    (System.Text.RegularExpressions.Regex """\d+""").Matches line |> 
        Seq.map 
            (fun m -> {
                ...
                row = row; 
                column = {|_start = m.Index; _end = m.Index + m.Value.Length - 1|}}) |> 
        Seq.toList
...
let concat3 (xs, ys, zs) =
    Seq.concat [xs; ys; zs]

let triplewise xs =
    let ys = prependAndAppend [] (Seq.map List.singleton xs)
    Seq.zip3 ys (Seq.tail ys) ((Seq.tail >> Seq.tail) ys) |> 
        Seq.map concat3
...
let validNumber (symbols: list<Symbol>) (number: Number) =
    Seq.exists (isAdjacent number) symbols
...
let filterNumbers (numbers: seq<list<Number>>) (symbols: seq<list<Symbol>>) =
    ...
    Seq.map2 (fun ns cs -> Seq.filter (validNumber cs) ns) numbers candidateSymbols |> Seq.concat
